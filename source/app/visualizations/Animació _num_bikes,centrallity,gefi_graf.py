...
mean_var = 2000
df2 = pd.DataFrame()
df2['Station Number'] = vals + \
    [x for x in smooth(vals, mean_var)[mean_var:-mean_var]]
df2[' '] = [datetime.datetime.fromtimestamp(x) for x in idxs.index] + [
    datetime.datetime.fromtimestamp(x) for x in idxs.index][mean_var:-mean_var]
df2['class'] = ['Station Number Over time' for _ in range(len(
    vals))] + ['Station Number Smoothed Over Time' for _ in range(len(vals)-2*mean_var)]


# In[ ]:


fig = px.line(df2, x=' ', y='Station Number', color="class",
              title="Network size on the last two years")
...
for i in range(len(df2)):
    ...
    for every_option in df2['id'].values[idxes][1: k+1]:
        ...
        if node in idxes2:
            if node2 not in graf.nodes():
                graf.add_node(node2, label=df2.iloc[idx]['address'])
...
dic_nodes_class = {'id': [], 'MClass': [], 'Betweenness Centrality': [
], "Eccentricity": [], "Strongly-Connected ID": [], "indegree": [], "outdegree": []}
...
for x in nodes:
    ...
    dic_nodes_class['Betweenness Centrality'].append(
        x[1]['Betweenness Centrality'])
    dic_nodes_class['Eccentricity'].append(x[1]['Eccentricity'])
    dic_nodes_class['Strongly-Connected ID'].append(
        x[1]['Strongly-Connected ID'])
...
df = df.groupby(by=["MClass", "last_updated"]).agg(
    ...
    mean_num_bikes_available_types_mechanical=(
        " num_bikes_available_types.mechanical", np.mean),
    max_num_bikes_available_types_mechanical=(
        " num_bikes_available_types.mechanical", "max"),
    min_num_bikes_available_types_mechanical=(
        " num_bikes_available_types.mechanical", "min"),
    sum_num_bikes_available_types_mechanical=(
        " num_bikes_available_types.mechanical", "sum"),

    mean_num_bikes_available_types_ebike=(
        " num_bikes_available_types.ebike", np.mean),
    max_num_bikes_available_types_ebike=(
        " num_bikes_available_types.ebike", "max"),
    min_num_bikes_available_types_ebike=(
        " num_bikes_available_types.ebike", "min"),
    sum_num_bikes_available_types_ebike=(
        " num_bikes_available_types.ebike", "sum"),
...
fig2 = px.line(df, x=' ', y="mean_bikes_available", color='class',
               title="Mean of available bikes through time per class")
x1 = datetime.datetime(year=2020, day=17, month=3)
x2 = datetime.datetime(year=2020, day=22, month=4)
fig = (go.Scatter(x=[x1, x1, x2, x2], y=[0, 200, 200, 0], name="COVID-19",
       fillcolor='red', line_color='red', fill='toself', opacity=0.1))
fig2.add_trace(fig)
...
x1 = datetime.datetime(year=2020, day=17, month=3)
x2 = datetime.datetime(year=2020, day=22, month=4)
fig2 = go.Figure((go.Scatter(x=[x1, x1, x2, x2], y=[
                 0, 200, 200, 0], fill="toself", opacity=0.1)))
fig2
...
fig = go.Figure([go.Scattermapbox(
    ...
    mode='markers', marker=go.scattermapbox.Marker(size=3), fill="toself", hovertext='District: '+names[i], name=names[i])
    for i in range(len(points_))] +

    [go.Scattermapbox(
        lat=[others[i][0]],
        lon=[others[i][1]],
        mode='markers', marker=go.scattermapbox.Marker(size=10), hovertext=str(i))
    ...
    [go.Scattermapbox(
        ...
        mode='text', text=['Class: '+str(x) for x in range(len(centers))], texttemplate='% {text: .10s} ',  textposition='top right',
             textfont=dict(size=16, color='black'), hovertext=['District: '+names[x] for x in range(len(centers))])])
