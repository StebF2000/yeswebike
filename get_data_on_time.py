...
class GetData:
	...
	def get_data1(self):
		...
		dic ={'AvailableMechanical':0, 'AvailableElectric':0,"ActiveBikes":0,'ActiveStations':0 }
		...
		for station in data['data']['stations']:
			...
			stations.append((station['station_id'],station['num_docks_available'],station['num_bikes_available']))
		
		with open("data_dashboard_top.json",'w') as hdlr:
			json.dump(dic,hdlr)
	...
	def get_data2(self):
		...
		self.data2 = {x['station_id'] : x['address'] for x in data2['data']['stations']}

		with open('station_information.json','w') as hdlr:
			json.dump(data2,hdlr)


	def get_data3(self,number_items=6):
    		
    		self.data1 = sorted(self.data1, key = lambda x: x[1],reverse=True)[:number_items]
    		print(self.data1)
    		print(self.data2)
    		res = [ (self.data2[x[0]] ,x[1] , x[2]) for x in self.data1]
    		res = { x[0] : (x[1],x[2]) for x in res}
    		print(res)
    		with open('dashboard_bottom_left.json','w') as hdlr:
    			json.dump(res,hdlr)
